...
import {
  ...
  completeChatGemini,
  isGeminiLLMModel,
  toGeminiChatCompletionRequestMessages,
  getThinkingBudget,
...
import {
  ...
  completeChatOpenAI,
  isOpenAILLMModel,
  toOpenAIChatCompletionRequestMessages,
  getReasoningEffort,
...
export const completeChatModel = async ({
  ...
  const weightForModel = (m: LLMModel): number => {
  ...
  const computeCostUnits = (usage: LLMTokenUsage, m: LLMModel): number => {
    const COST_SCALE = 10;
  ...
  if (isGeminiLLMModel(model)) {
    ...
    const thinkingBudget = getThinkingBudget(model, thinking);
